# -*- coding: utf-8 -*-
"""
Created on Fri Dec 10 18:42:59 2021
...
from Fair_Metric import fair_metric
...
#Dataset= 'bail'
Dataset='credit'
#df = pd.read_csv('german.emb',delimiter=' ',sep='\t',skiprows=[0],names=list(np.arange(100)),index_col=0)  
df = pd.read_csv(Dataset+'_swap.emb',delimiter=' ',sep='\t',skiprows=[0],names=list(np.arange(100)),index_col=0)  
df.describe()
print(df.shape)
sens=np.load(Dataset+'_sens.npy')
# df=df.drop(['FixSeqDSST','FixSeqOTDS'],axis=1)
labels =np.load(Dataset+'_y.npy')  # interchange 0's to 1's
"""
...
kf = StratifiedKFold(n_splits=8, shuffle=False)
for train_index, test_index in kf.split(df, labels):
    ...
    rf = RandomForestRegressor(n_estimators=80, random_state=42) #n_estimators=100
    ...
    print('Fairness_metric',fair_metric(result_trial,Y_test,Y_sens))
