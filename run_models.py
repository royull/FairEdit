...
from models.nifty.utils import *
from models.nifty import *
...
for epoch in range(args.epochs+1):
    ...
    if args.model in ['gcn', 'sage']:
        ...
        if loss_val.item() < best_loss:
            ...
            torch.save(model.state_dict(), 'weights/weights_{0}_{1}.pt'.format(args.dataset, args.model))
        ...
        if (val_c_loss + val_s_loss) < best_loss:
            ...
            torch.save(model.state_dict(), f'weights_ssf_{args.encoder}_{0}_{1}.pt'.format(args.dataset, args.model))

# print("Optimization Finished!")
# print("Total time elapsed: {:.4f}s".format(time.time() - t_total))

if args.model in ['gcn', 'sage']:
    model.load_state_dict(torch.load('weights/weights_{0}_{1}.pt'.format(args.dataset, args.model)))
    model.eval()
...
dict_vals = {}
dict_vals['labels'] = labels
dict_vals['outputs'] = output
dict_vals['counter_output'] = counter_output
dict_vals['noisy_output'] = noisy_output
dict_vals['sens'] = sens
dict_vals['idx_test'] = idx_test
with open('results/results_{0}_{1}.pickle'.format(args.dataset, args.model), 'wb') as handle:
    pickle.dump(dict_vals, handle, protocol=pickle.HIGHEST_PROTOCOL)
